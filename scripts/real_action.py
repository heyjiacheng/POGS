...
class RobotController:
    ...
    def execute_task(self, instruction=None, rekep_program_dir=None):
        ...
        self._run_all_stages(rekep_program_dir)

    def _run_all_stages(self, rekep_program_dir):
        """Run all stages and collect subgoals"""
        # Load program info and constraints once
        self.program_info = self._load_program_info(rekep_program_dir)
        self.constraint_fns = self._load_constraints(rekep_program_dir)
        
        # Setup environment state once
        ...
        # Process each stage
        num_stages = self.program_info['num_stages']
        ...
        for stage in range(1, num_stages + 1):
            ...
            # Generate subgoal for this stage
            subgoal = self._generate_subgoal()
            print(f"Stage {stage} subgoal: {subgoal}")
            
            # Store subgoal info
            subgoal_info = {
                "stage": stage,
                "subgoal_pose": subgoal.tolist(),
                "current_ee_pose": self.curr_ee_pose.tolist(),
                "is_grasp_stage": self.is_grasp_stage,
                "is_release_stage": self.is_release_stage
            }
            self.all_subgoals.append(subgoal_info)
            
            # Generate actions for this stage (optional - for completeness)
            actions = self._generate_path(subgoal)
            
            # Save stage-specific data
            self._save_stage_data(actions, subgoal_info, stage, rekep_program_dir)
            
            # Update ee pose for next stage (simulate movement)
            self.curr_ee_pose = subgoal.copy()
            # CRITICAL: Update the first keypoint (index 0) which represents the end effector position
            self.keypoints[0] = subgoal[:3]  # Only update position, not orientation
        
        # Save all subgoals summary
        self._save_all_subgoals(rekep_program_dir)
    ...
    def _generate_actions(self):
        ...
        # Store subgoal for later saving
        self.current_subgoal = next_subgoal
    ...
    def _save_actions(self, actions, stage, rekep_program_dir):
        """Save action sequence (legacy method, not used in new pipeline)"""
        pass

    def _save_stage_data(self, actions, subgoal_info, stage, rekep_program_dir):
        """Save data for a specific stage (simplified)"""
        pass

    def _save_all_subgoals(self, rekep_program_dir):
        """Save summary of all subgoals"""
        # Save to outputs directory only
        all_subgoals_path = os.path.join('./outputs', 'all_subgoals.json')
        os.makedirs(os.path.dirname(all_subgoals_path), exist_ok=True)
        
        summary_data = {
            "num_stages": len(self.all_subgoals),
